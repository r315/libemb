...
static huart_t huart1, huart2, huart3, huart4;
...
void UART_Init(serialbus_t *serialbus){
    ...
    switch(serialbus->bus){
        ...
        case UART_BUS3:
            ...
            huart->usart = USART4;
        #if UART_RX_MODE == UART_MODE_DMA
            rx_req = DMA1_REQ_USART4_RX;
        #endif
        #if UART_TX_MODE == UART_MODE_DMA
            tx_req = DMA1_REQ_USART4_TX;
        #endif
        #if UART_RX_MODE == UART_MODE_FIFO || UART_TX_MODE == UART_MODE_FIFO
            irq = USART3_4_IRQn;
        #endif
...
static void UART_IRQHandler(huart_t *huart)
{
    ...
    uint32_t errorflags = status & (uint32_t)(USART_ISR_PE | USART_ISR_FE | USART_ISR_ORE | USART_ISR_NE);
    ...
    if ((usart->CR1 & USART_CR1_RE) && (status & USART_ISR_RXNE)){
        fifo_put(&huart->rxfifo, (uint8_t)usart->RDR);
    }
#endif
#if UART_TX_MODE == UART_MODE_FIFO
    // Handle data to be transmitted
    if ((usart->CR1 & USART_CR1_TXEIE) && (status & USART_ISR_TXE)){
    ...
    //usart->ICR = (USART_ICR_RXNE | USART_ISR_TXE | USART_ISR_CTS | USART_ISR_LBD);
}
