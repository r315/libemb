...
uint32_t SPI_Init(spibus_t *spi){
	uint32_t data;
	uint8_t ck, psel;
	...
	data = (spi->cfg & SPI_CFG_TRF_16BIT) ? SSP_CR0_DSS_16 : SSP_CR0_DSS_8;
...
uint16_t SPI_Xchg(spibus_t *spi, uint8_t *data){
	LPC_SSP_TypeDef *sspx = (LPC_SSP_TypeDef*)spi->handle;
	sspx->DR = *data;
	while((sspx->SR & SSP_SR_BSY)){}
	return sspx->DR;
