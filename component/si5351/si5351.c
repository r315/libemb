/*
 * si5351.c - Si5351 generic library based on Jason Milldrum and Dana H. Myers library for arduino
 *
...
static Si5351Status_t dev_status;
 ...
 * @return uint8_t
 */
static uint8_t write_reg_bulk(uint8_t addr, uint8_t *data, uint8_t bytes)
{
    data[0] = addr;
    return I2C_Write(m_i2c, SI5351_BUS_BASE_ADDR, data, bytes + 1);
}

static uint8_t write_reg(uint8_t addr, uint8_t data)
{
    uint8_t _data[2];
    _data[0] = addr;
    _data[1] = data;
 ...
 */
static void set_pll(uint64_t pll_freq, enum si5351_pll target_pll)
{
  uint8_t bulk_params[20];
  ...
  uint8_t i = 1;    // [0] index is reserved for register address
  uint8_t temp;
  ...
  if(target_pll == SI5351_PLLA)
  {
    write_reg_bulk(SI5351_PLLA_PARAMETERS, bulk_params, i);
	plla_freq = pll_freq;
  }
  else if(target_pll == SI5351_PLLB)
  {
    write_reg_bulk(SI5351_PLLB_PARAMETERS, bulk_params, i);
	pllb_freq = pll_freq;
  }
}
 ...
 */
static void set_integer(enum si5351_clock clk, uint8_t enable)
{
	uint8_t reg_val;
	reg_val = read_reg(SI5351_CLK0_CTRL + (uint8_t)clk);
	...
	write_reg(SI5351_CLK0_CTRL + (uint8_t)clk, reg_val);
}
 ...
 *   Set to 1 to enable, 0 to disable
 */
static void set_ms(enum si5351_clock clk, struct Si5351RegSet ms_reg, uint8_t int_mode, uint8_t r_div, uint8_t div_by_4)
{
	uint8_t i = 1;    // [0] index is reserved for register address
 	uint8_t temp;
 	uint8_t reg_val;
	...
	{
		case SI5351_CLK0:
			write_reg_bulk(SI5351_CLK0_PARAMETERS, bulk_params, i);
			set_integer(clk, int_mode);
			ms_div(clk, r_div, div_by_4);
			break;
		case SI5351_CLK1:
			write_reg_bulk(SI5351_CLK1_PARAMETERS, bulk_params, i);
			set_integer(clk, int_mode);
			ms_div(clk, r_div, div_by_4);
			break;
		case SI5351_CLK2:
			write_reg_bulk(SI5351_CLK2_PARAMETERS, bulk_params, i);
			set_integer(clk, int_mode);
			ms_div(clk, r_div, div_by_4);
			break;
		case SI5351_CLK3:
			write_reg_bulk(SI5351_CLK3_PARAMETERS, bulk_params, i);
			set_integer(clk, int_mode);
			ms_div(clk, r_div, div_by_4);
			break;
		case SI5351_CLK4:
			write_reg_bulk(SI5351_CLK4_PARAMETERS, bulk_params, i);
			set_integer(clk, int_mode);
			ms_div(clk, r_div, div_by_4);
			break;
		case SI5351_CLK5:
			write_reg_bulk(SI5351_CLK5_PARAMETERS, bulk_params, i);
			set_integer(clk, int_mode);
			ms_div(clk, r_div, div_by_4);
			break;
		case SI5351_CLK6:
			write_reg(SI5351_CLK6_PARAMETERS, temp);
			ms_div(clk, r_div, div_by_4);
			break;
		case SI5351_CLK7:
			write_reg(SI5351_CLK7_PARAMETERS, temp);
			ms_div(clk, r_div, div_by_4);
			break;
	}
}
 ...
 * crystal.
 */
static void set_correction(int32_t corr, enum si5351_pll_input ref_osc)
{
 ...
 */
static void pll_reset(enum si5351_pll target_pll)
{
	if(target_pll == SI5351_PLLA)
 	{
 ...
 */
static void set_ref_freq(uint32_t ref_freq, enum si5351_pll_input ref_osc)
{
	// uint8_t reg_val;
 ...
 *
 */
static void set_xtal_cl(enum si5351_xtal_cl cl)
{
    uint8_t reg_val = ((uint8_t)cl << 6);
    write_reg(SI5351_CRYSTAL_LOAD, reg_val | SI5351_CRYSTAL_LOAD_RSVD);
}
 ...
 */
static void set_phase(enum si5351_clock clk, uint8_t phase)
{
	// Mask off the upper bit since it is reserved
	phase = phase & 0b01111111;

	write_reg(SI5351_CLK0_PHASE_OFFSET + (uint8_t)clk, phase);
}
 ...
 */
static void set_pll_input(enum si5351_pll pll, enum si5351_pll_input input)
{
	uint8_t reg_val;
 ...
 */
static void set_vcxo(uint64_t pll_freq, uint8_t ppm)
{
	struct Si5351RegSet pll_reg;
	uint64_t vcxo_param;
    uint8_t bulk_params[20];
	...
	write_reg(SI5351_VXCO_PARAMETERS_LOW, temp);

	temp = (uint8_t)((vcxo_param >> 8) & 0xFF);
	write_reg(SI5351_VXCO_PARAMETERS_MID, temp);

	temp = (uint8_t)((vcxo_param >> 16) & 0x3F);
	write_reg(SI5351_VXCO_PARAMETERS_HIGH, temp);
}
 ...
 * in RAM.
 */
static int32_t get_correction(enum si5351_pll_input ref_osc)
{
 ...
 *
 */
uint8_t Si5351_Init(i2cbus_t *i2c, uint32_t xo_freq, int32_t corr)
{
	uint8_t reg_val;
...
void Si5351_Reset(void)
{
    ...
    for(uint8_t i = 0; i < 8; i++){
	    Si5351_MsSourceSet((enum si5351_clock)(SI5351_CLK0 + i), pll_assignment[i]);
    }
 ...
 *   (use the si5351_clock enum)
 */
uint8_t Si5351_FreqSet(enum si5351_clock clk, uint64_t freq)
{
 ...
 *   (use the si5351_clock enum)
 */
uint8_t Si5351_FreqSetManual(enum si5351_clock clk, uint64_t freq, uint64_t pll_freq)
{
	struct Si5351RegSet ms_reg;
	uint8_t int_mode = 0;
	uint8_t div_by_4 = 0;
...
/*
 * set_output_enable(enum si5351_clock clk, uint8_t enable)
 *
 ...
 * enable - Set to 1 to enable, 0 to disable
 */
void Si5351_OutputEnableSet(enum si5351_clock clk, uint8_t enable)
{
  uint8_t reg_val;
 ...
 */
void Si5351_OutputDriveStrengthSet(enum si5351_clock clk, enum si5351_drive drive)
{
    uint8_t reg_val;
    reg_val = read_reg(SI5351_CLK0_CTRL + (uint8_t)clk) & ~SI5351_CLK_DRIVE_STRENGTH_MASK;
    write_reg(SI5351_CLK0_CTRL + (uint8_t)clk, reg_val | ((uint8_t)drive & SI5351_CLK_DRIVE_STRENGTH_MASK));
}
 ...
 */
void Si5351_OutputDisableStateSet(enum si5351_clock clk, enum si5351_clock_output_disable dis_state)
{
    ...
    reg_addr = (clk < SI5351_CLK4) ? SI5351_CLK3_0_DISABLE_STATE : SI5351_CLK7_4_DISABLE_STATE;
 ...
 * saving feature).
 */
void Si5351_ClockPwrSet(enum si5351_clock clk, uint8_t pwr)
{
	uint8_t reg_val;
	reg_val = read_reg(SI5351_CLK0_CTRL + (uint8_t)clk);
 ...
 */
void Si5351_ClockInvertSet(enum si5351_clock clk, uint8_t inv)
{
	uint8_t reg_val;
	reg_val = read_reg(SI5351_CLK0_CTRL + (uint8_t)clk);
 ...
 * the clock output.
 */
void Si5351_ClockSourceSet(enum si5351_clock clk, enum si5351_clock_source src)
{
	uint8_t reg_val;
	reg_val = read_reg(SI5351_CLK0_CTRL + (uint8_t)clk);
	...
	switch(src)
	{
        ...
        case SI5351_CLK_SRC_MS0:
            //if(clk == SI5351_CLK0 || clk == SI5351_CLK4){
            //    return;
            //}
...
/*
 * set_clock_rdiv(enum si5351_clock, uint8_t)
 *
 * Sets the final integer divider
 *
 * clk - Clock output
 *   (use the si5351_clock enum)
 * r_div - divider
 *   (use si5351_clock_r_div enum)
*
 */
void Si5351_ClockRdivSet(enum si5351_clock clk, enum si5351_clock_r_div r_div)
{
    uint8_t reg_val;
    uint8_t addr = SI5351_CLK0_PARAMETERS + ((uint8_t)clk * 8) + 2;
    reg_val = read_reg(addr) & ~SI5351_OUTPUT_CLK_DIV_MASK;
    write_reg(addr, reg_val | (((uint8_t)r_div << SI5351_OUTPUT_CLK_DIV_SHIFT) & SI5351_OUTPUT_CLK_DIV_MASK));
}
 ...
 */
void Si5351_FanoutSet(enum si5351_fanout fanout, uint8_t enable)
{
	uint8_t reg_val, mask;

	reg_val = read_reg(SI5351_FANOUT_ENABLE);

    mask = (uint8_t)fanout & (SI5351_FANOUT_CLKIN | SI5351_FANOUT_XO | SI5351_FANOUT_MS);

    reg_val = (enable) ? (reg_val | mask) : (reg_val & ~mask);

	write_reg(SI5351_FANOUT_ENABLE, reg_val);
}
 ...
 */
void Si5351_MsSourceSet(enum si5351_clock clk, enum si5351_pll pll)
{
	uint8_t reg_val;
